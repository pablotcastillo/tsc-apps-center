    ...
    const initialNewsData = [
      { id: 'news1', title: 'Lanzamiento de Gestor de Proyectos TSC v2.5', date: '2025-05-15', category: 'Lanzamientos', summary: 'Estamos emocionados de anunciar la nueva versión de nuestro Gestor de Proyectos, con mejoras de rendimiento y nuevas funcionalidades de colaboración.', appLink: '/apps/app1', imageDescription: 'Equipo celebrando el lanzamiento de un nuevo software.' },
      { id: 'news2', title: 'TSC Innovation reconocida por su excelencia en IA', date: '2025-05-10', category: 'Premios', summary: 'TSC Innovation ha recibido un prestigioso premio por su contribución al campo de la Inteligencia Artificial con nuestra herramienta Analizador de Datos IA.', appLink: '/apps/app2', imageDescription: 'Trofeo de premio sobre una mesa elegante.' },
      { id: 'news3', title: 'Próximo Webinar: Maximizando la eficiencia con CRM Conecta', date: '2025-05-05', category: 'Eventos', summary: 'Únase a nuestro webinar gratuito el 20 de Mayo para descubrir cómo CRM TSC Conecta puede transformar sus procesos de venta y relación con clientes.', appLink: '/apps/app3', imageDescription: 'Persona presentando en un webinar online.' },
      { id: 'news4', title: 'Actualización de seguridad importante para todas las apps', date: '2025-04-25', category: 'Seguridad', summary: 'Hemos implementado una actualización de seguridad crítica en todas nuestras aplicaciones para garantizar la protección de sus datos. Recomendamos actualizar a la brevedad.', appLink: '/apps', imageDescription: 'Icono de escudo de seguridad digital.' },
    ];
    ...
    const NewsCard = ({ item, index }) => (
      ...
      >
        <div className="md:w-1/3 h-48 md:h-auto rounded-md overflow-hidden">
          <img  alt={item.title} class="w-full h-full object-cover"  src="https://images.unsplash.com/photo-1697256200022-f61abccad430" />
        </div>
        <div className="md:w-2/3 flex flex-col">
          <h3 className="text-2xl font-semibold text-primary mb-2">{item.title}</h3>
          <div className="flex items-center text-sm text-muted-foreground mb-3 space-x-4">
            <span className="flex items-center"><Calendar size={16} className="mr-1.5" /> {item.date}</span>
            <span className="flex items-center"><Tag size={16} className="mr-1.5" /> {item.category}</span>
          </div>
          <p className="text-muted-foreground mb-4 flex-grow">{item.summary}</p>
          <a href={item.appLink} className="self-start">
