    ...
    import AdminDashboard from './AdminDashboard'; // asegúrate de importar


    const FeatureCard = ({ icon, title, description, linkTo, linkText }) => (
      <motion.div
        className="bg-card p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-500 flex flex-col"
...
const HomePage = ({ showAdmin }) => {
  return (
    <div className="space-y-12">

      {/* Panel de Administración (solo si aplica) */}
      {showAdmin && (
        <section className="container mx-auto px-6 py-4">
          <div className="border border-primary rounded-lg shadow-md p-4 bg-white dark:bg-slate-900">
            <AdminDashboard />
          </div>
        </section>
      )}
      ...
      <section className="container mx-auto px-6 py-12">
        <div className="bg-card p-8 rounded-lg shadow-lg text-center">
          <img-replace alt="Team working on innovative solutions" className="w-full max-w-md mx-auto rounded-md mb-6 shadow-md" />
